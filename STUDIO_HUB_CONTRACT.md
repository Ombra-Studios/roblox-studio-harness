...
- **Numele proiectului** vine din fereastra Studio, nu din `game.Name`: `studio_place_name("Ball (placeId: 123)") == "Ball"`. `game.Name` este „Place1” pentru toate locurile unui univers, deci două proiecte ar apărea identic în plugin, în panou și la colegi. Când instanța MCP nu poate fi determinată, rămâne `place_name` raportat de plugin.
- **Nimic nu este tăcut**: o sesiune din terminal care nu a ales singură, dar moștenește „Studio țintă” (`/v1/default-studio`) sau singura fereastră deschisă, primește în flux un eveniment `status` cu `tool: "studio_use"` — „Sesiunea lucrează în <fereastră>. Schimbă proiectul cu studio_use.” — o dată per fereastră moștenită.
